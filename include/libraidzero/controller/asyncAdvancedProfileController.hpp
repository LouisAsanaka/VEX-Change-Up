...
public:
    ...
    AsyncAdvancedProfileController &operator=(AsyncAdvancedProfileController &&other) = delete;

    ~AsyncAdvancedProfileController() override;
     ...
     */
    void generatePath(std::initializer_list<planner::UnitableWaypoint> iwaypoints, const std::string &ipathId);
     ...
     * @param iwaypoints The waypoints to hit on the path.
     * @param ipathId A unique identifier to save the path with.
     * @param iconfig The config to use for this path only.
     */
    void generatePath(std::initializer_list<planner::UnitableWaypoint> iwaypoints,
    ...
    /**
     * Gets the identifiers of all paths saved in this `AsyncAdvancedProfileController`.
     *
     ...
     * @param imirrored Whether to follow the profile mirrored.
     */
    void moveTo(std::initializer_list<planner::UnitableWaypoint> iwaypoints,
     ...
     */
    planner::UnitableWaypoint getError() const override;
     ...
     * by the `AsyncAdvancedProfileControllerBuilder` when making a new instance of this class.
     */
    void startThread();
    ...
    std::map<std::string, planner::MotionProfile> paths{};
     ...
     */
    virtual void executeSinglePath(const planner::MotionProfile& profile, std::unique_ptr<AbstractRate> rate);
    ...
    /**
     * Reads the length of the profile in a thread-safe manner.
     *
     * @param profile The profile to read from.
     * @return The length of the profile.
     */
    int getProfileLength(const planner::MotionProfile& profile);
};
